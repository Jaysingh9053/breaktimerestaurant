...
class OrderDialog(ThemedDialog):
    def __init__(
        ...
        customers,
        current_user=None,
        restrict_to_current_user=False,
        items_only=False,
        parent=None,
    ):
        dialog_title = "Add Items" if items_only else "Create Order"
        dialog_subtitle = (
            "Append more items to the selected unpaid order."
            if items_only
            else "Choose where the order belongs, add items, and assign the responsible staff member."
        )
        super().__init__(dialog_title, dialog_subtitle, width=700, parent=parent)
        self.items_only = items_only
        self.menu_items = [
            item
            for item in menu_items
            if int(item["is_active"]) == 1 and (int(item["is_stock_tracked"]) == 0 or int(item["stock_qty"]) > 0)
        ]
        self.menu_item_lookup = {int(item["id"]): dict(item) for item in self.menu_items}
        self.item_categories = sorted({str(item["category"]).strip() for item in self.menu_items if str(item["category"]).strip()})
        ...
        waiter_users = [user for user in self.users if str(user.get("role", "")).strip().lower() == "waiter"]
        self.waiter_users = waiter_users or self.users
        ...
        item_intro = QLabel(
            "Add items to the order below. Selecting the same item again updates its quantity in the current order instead of creating a duplicate row."
            if not self.items_only
            else "Select category or search by item name, then add only the extra items you want to append to this unpaid order."
        )
        ...
        self.item_category_input.addItem("All Categories")
        for category in self.item_categories:
            self.item_category_input.addItem(category)
        configure_combo_box(self.item_category_input, min_chars=14, popup_width=240)
        self.item_category_input.setStyleSheet(input_style)
        self.item_category_input.setToolTip("Filter menu items by category")
        self.item_search_input = QLineEdit()
        self.item_search_input.setPlaceholderText("Search item by name")
        self.item_search_input.setStyleSheet(input_style)
        self.item_search_input.setToolTip("Type part of the item name to filter the menu list")
        ...
        item_row.addWidget(QLabel("Category"), 0, 0)
        item_row.addWidget(QLabel("Search By Name"), 0, 1)
        item_row.addWidget(self.item_category_input, 1, 0)
        item_row.addWidget(self.item_search_input, 1, 1, 1, 3)
        item_row.addWidget(QLabel("Menu Item"), 2, 0)
        item_row.addWidget(QLabel("Item ID"), 2, 1)
        item_row.addWidget(QLabel("Unit Price"), 2, 2)
        item_row.addWidget(QLabel("Qty To Add"), 2, 3)
        item_row.addWidget(self.item_input, 3, 0)
        item_row.addWidget(self.item_id_input, 3, 1)
        item_row.addWidget(self.item_price_input, 3, 2)
        item_row.addWidget(self.quantity_input, 3, 3)
        item_row.addWidget(self.item_hint_label, 4, 0, 1, 4)
        item_row.addWidget(self.item_action_label, 5, 0, 1, 4)
        item_row.addWidget(add_button, 6, 2)
        item_row.addWidget(remove_button, 6, 3)
        ...
        buttons = self.create_button_box("Add Items" if self.items_only else "Create Order")
        buttons.accepted.connect(self.validate_and_accept)
        buttons.rejected.connect(self.reject)
        self.content_layout.addWidget(buttons)
        self.item_category_input.currentTextChanged.connect(self._handle_item_filter_change)
        self.item_search_input.textChanged.connect(self._handle_item_filter_change)
    ...
    def _current_menu_item(self):
        ...
        if menu_item_id is None:
            return None
        return self.menu_item_lookup.get(int(menu_item_id))

    def _filtered_menu_items(self):
        category = self.item_category_input.currentText().strip()
        search_text = self.item_search_input.text().strip().lower()
        filtered_items = []
        for item in self.menu_items:
            item_category = str(item.get("category", "")).strip()
            item_name = str(item.get("name", ""))
            if category and category != "All Categories" and item_category != category:
                continue
    ...
    def _refresh_item_options(self, selected_menu_item_id=None):
        previous_selection = selected_menu_item_id
        if previous_selection is None:
            previous_selection = self.item_input.currentData()
        filtered_items = self._filtered_menu_items()
        self.item_input.blockSignals(True)
        ...
        for item in filtered_items:
            ...
            if previous_selection is not None and int(item["id"]) == int(previous_selection):
                selected_index = self.item_input.count() - 1
        if self.item_input.count():
            self.item_input.setCurrentIndex(selected_index if selected_index >= 0 else 0)
        self.item_input.blockSignals(False)
        self._sync_item_selection_state()

    def _handle_item_filter_change(self, *_args):
        self._refresh_item_options()
    ...
    def refresh_totals(self):
        ...
        if self.items_only:
            self.summary_label.setText(
                f"Items To Add: {format_currency(totals['subtotal'])} | "
                f"GST Impact Estimate: {format_currency(totals['tax_amount'])} | "
                f"Added Total Estimate: {format_currency(totals['grand_total'])}"
            )
            return
        self.summary_label.setText(
...
class BillPreviewDialog(ThemedDialog):
    ...
    def print_bill(self):
        ...
        if dialog.exec() == QDialog.DialogCode.Accepted:
            ...
            self.bill_printed = True
            self.accept()


class PaymentMethodDialog(ThemedDialog):
    def __init__(self, order_number, parent=None):
        super().__init__("Make Payment", f"Select payment method for {order_number}.", width=360, parent=parent)
        self.selected_method = None

        helper = QLabel("Choose one payment method. Bill preview will open immediately after selection.")
        helper.setWordWrap(True)
        helper.setStyleSheet("color: #6b584c; font-size: 13px;")
        self.content_layout.addWidget(helper)

        buttons = QVBoxLayout()
        buttons.setSpacing(10)
        for method in ("Cash", "Card", "UPI"):
            button = QPushButton(f"{method} Payment")
            style_button(button, "primary")
            button.clicked.connect(lambda _checked=False, payment_method=method: self._select_method(payment_method))
...
class DashboardWindow(QMainWindow):
    ...
    def _build_shell(self):
        ...
        title.setStyleSheet("color: #f6ece6; background: transparent; border: none; font-size: 18px; font-weight: 700; padding: 12px 10px;")
        ...
        profile_name.setStyleSheet("color: white; background: transparent; border: none; font-size: 15px; font-weight: 700;")
        profile_role = QLabel(self.current_user.get("role", "Staff"))
        profile_role.setStyleSheet("color: #f3d7c2; background: transparent; border: none; font-size: 12px;")
        ...
        help_label.setStyleSheet("color: #5a3a2f; background: transparent; border: none; font-size: 13px; font-weight: 700; padding: 4px 8px;")
        help_label.setToolTip("Contact the administrator: adminbreaktime@break.com")
        top_row.addWidget(help_label)
    ...
    def _build_dashboard_page(self):
        ...
        recent_badges.addWidget(BadgeLabel(f"{self.metrics['pending_billing_orders']} pending billing", "#4f8c2f"))
    ...
    def _build_orders_page(self):
        ...
        layout.addLayout(self._cards_row([
            ...
            ("Pending Billing", str(self.metrics["pending_billing_orders"]), "#4f8c2f"),
            ("Unpaid Orders", str(active_unpaid), "#d9881f"),
        ]))
        order_buttons = []
        if self._can("create_order"):
            order_buttons.append(("New Order", self.create_order, "primary"))
            order_buttons.append(("Add Items", self.add_items_to_selected_order, "secondary"))
    ...
    def _build_billing_page(self):
        ...
        if self._can("record_payment"):
            billing_buttons.append(("Make Payment", self.record_selected_payment, "secondary"))
    ...
    def add_items_to_selected_order(self):
        if not self._can("create_order"):
            QMessageBox.information(self, "Access", "You do not have permission to edit orders.")
            return
        order_id = self._selected_id(self.orders_table)
        if order_id is None:
            QMessageBox.information(self, "Orders", "Select an order first.")
            return
        selected_order = next((order for order in self.orders if int(order["id"]) == int(order_id)), None)
        if not selected_order:
            QMessageBox.warning(self, "Orders", "Selected order could not be loaded.")
            return
        if selected_order["payment_status"] == "Paid":
            QMessageBox.information(self, "Orders", "Paid orders cannot be edited.")
            return
        if selected_order["status"] == "Cancelled":
            QMessageBox.information(self, "Orders", "Cancelled orders cannot be edited.")
            return
        ...
        if dialog.exec() != QDialog.DialogCode.Accepted:
            return
        try:
            self.database.add_items_to_order(order_id, dialog.get_payload()["items"])
        except Exception as error:
            QMessageBox.warning(self, "Orders", str(error))
            return
        self.refresh_pages("Orders")
    ...
    def record_selected_payment(self):
        ...
        if order_id is None:
            ...
            return
        selected_order = next((order for order in self.orders if int(order["id"]) == int(order_id)), None)
        payment_dialog = PaymentMethodDialog(
            selected_order["order_number"] if selected_order else f"Order #{order_id}",
            parent=self,
        )
        if payment_dialog.exec() != QDialog.DialogCode.Accepted or not payment_dialog.selected_method:
            return
        method = payment_dialog.selected_method
        invoice = self.database.get_order_invoice(order_id)
        if not invoice:
        ...
        invoice["paid_at"] = datetime.now().replace(microsecond=0).isoformat(sep=" ")
        preview = BillPreviewDialog(invoice, parent=self)
        if preview.exec() != QDialog.DialogCode.Accepted or not preview.bill_printed:
            return
        try:
