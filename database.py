...
class DatabaseManager:
    ...
    def add_items_to_order(self, order_id: int, items: list[dict]) -> None:
        if not items:
            raise ValueError("At least one item is required to update the order.")

        with self._connect() as connection:
            order = connection.execute(
                """
                SELECT id, status, payment_status, offer_id
                FROM sales_orders
                WHERE id = ?
                """,
                (int(order_id),),
            ...
            if order["payment_status"] == "Paid":
                raise ValueError("Paid orders cannot be edited.")
            ...
            for item in items:
                ...
                if quantity <= 0:
                    raise ValueError("Item quantity must be greater than zero.")
                item_totals[menu_item_id] = item_totals.get(menu_item_id, 0) + quantity

            menu_ids = list(item_totals.keys())
            placeholders = ",".join("?" for _ in menu_ids)
            menu_rows = connection.execute(
                f"SELECT id, name, price, stock_qty, is_stock_tracked, is_active FROM menu_items WHERE id IN ({placeholders})",
                menu_ids,
            ).fetchall()
            menu_map = {int(row["id"]): dict(row) for row in menu_rows}

            for menu_item_id, quantity in item_totals.items():
                menu_item = menu_map.get(menu_item_id)
                if not menu_item or int(menu_item["is_active"]) != 1:
                    raise ValueError("Selected menu item is unavailable.")
                if int(menu_item["is_stock_tracked"]) == 1 and int(menu_item["stock_qty"]) < quantity:
            ...
            for menu_item_id, quantity in item_totals.items():
                ...
                if existing_item:
                    ...
                    connection.execute(
                        ...
                        WHERE id = ?
                        """,
                        (updated_quantity, round(unit_price * updated_quantity, 2), int(existing_item["id"])),
                    )
                else:
                    connection.execute(
                        """
                        ...
                        VALUES (?, ?, ?, ?, ?, ?)
                        """,
                        (int(order_id), int(menu_item_id), menu_item["name"], quantity, unit_price, line_total),
                    )

                if int(menu_item["is_stock_tracked"]) == 1:
                    connection.execute(
                        "UPDATE menu_items SET stock_qty = stock_qty - ? WHERE id = ?",
                        (quantity, int(menu_item_id)),
                    )
    ...
    def update_order_status(self, order_id: int, status: str) -> None:
        with self._connect() as connection:
            ...
            if current_status in {"Preparing", "Ready"}:
            ...
            connection.execute("UPDATE sales_orders SET status = ? WHERE id = ?", (status, int(order_id)))
            connection.commit()
    ...
    def record_payment(self, order_id: int, method: str) -> None:
        with self._connect() as connection:
            order = connection.execute(
                """
                SELECT id, total_amount, discount_amount, offer_name, payment_status, status, order_type
                FROM sales_orders
                WHERE id = ?
                """,
                (int(order_id),),
            ...
            if order["order_type"] == "Dine In" and order["status"] != "Served":
                raise ValueError("Dine-in orders can be billed only after they are marked served.")
    ...
    def get_unpaid_orders(self) -> list[dict]:
        return [
            ...
            and (order["order_type"] != "Dine In" or order["status"] == "Served")
        ]
    ...
    def get_kitchen_queue(self, limit: int = 5) -> list[dict]:
        with self._connect() as connection:
            rows = connection.execute(
                ...
                JOIN users u ON u.id = so.staff_id
                LEFT JOIN order_items oi ON oi.order_id = so.id
                WHERE so.status IN ('Pending', 'Served')
                GROUP BY so.id
                ORDER BY datetime(so.created_at) DESC
                LIMIT ?
                """,
    ...
    def get_dashboard_metrics(self) -> dict:
        with self._connect() as connection:
            ...
            active_orders = connection.execute(
                "SELECT COUNT(*) FROM sales_orders WHERE status IN ('Pending', 'Served')"
            ).fetchone()[0]
            pending_billing_orders = connection.execute(
                "SELECT COUNT(*) FROM sales_orders WHERE status = 'Served' AND payment_status != 'Paid'"
            ).fetchone()[0]
        ...
        return {
            ...
            "pending_billing_orders": int(pending_billing_orders or 0),
